...
class CaloricallyPerfectGas(GasModel):
	...
	def __init__(self, gasDict):
		...
		self.const_visc_idxs = \
			np.squeeze(np.argwhere(self.temp_ref < 1.0e-7), axis=1)
		self.suth_visc_idxs = \
			np.squeeze(np.argwhere(self.temp_ref >= 1.0e-7), axis=1)
	...
	def calc_mix_gas_constant(self, mass_fracs):
		...
		r_mix = R_UNIV * ((1.0 / self.mol_weights[-1])
	...
	def calc_spec_enth(self, temperature):
		...
		spec_enth = (self.cp[:, None]
					* np.repeat(np.reshape(temperature, (1, -1)),
					self.num_species_full, axis=0) + self.enth_ref[:, None])
	...
	def calc_stag_enth(self, velocity, mass_fracs,
		...
		if spec_enth is None:
			assert (temperature is not None), \
				"Must provide temperature if not providing species enthalpies"
	...
	def calc_species_dynamic_visc(self, temperature):
		...
		spec_dyn_visc = \
			np.zeros((self.num_species_full, len(temperature)), dtype=REAL_TYPE)
	...
	def calc_mix_dynamic_visc(self, spec_dyn_visc=None, temperature=None,
		...
		if spec_dyn_visc is None:
			assert (temperature is not None), \
				"Must provide temperature if not providing species dynamic viscosities"
		...
		if self.num_species_full == 1:
			...
			if mole_fracs is None:
				assert (mass_fracs is not None), \
					"Must provide mass fractions if not providing mole fractions"
				mole_fracs = self.calc_all_mole_fracs(mass_fracs, mw_mix)

			phi = \
				np.zeros((self.num_species_full, spec_dyn_visc.shape[1]), dtype=REAL_TYPE)
			for spec_idx in range(self.num_species_full):

				muFac = np.sqrt(spec_dyn_visc[[spec_idx], :] / spec_dyn_visc)
				phi[spec_idx, :] = \
					np.sum(mole_fracs * np.square(1.0
					+ muFac * self.mix_mass_matrix[[spec_idx], :].T)
					* self.mix_inv_mass_matrix[[spec_idx], :].T, axis=0)
	...
	def calc_mix_thermal_cond(self, spec_therm_cond=None, spec_dyn_visc=None,
		...
		if spec_therm_cond is None:
			...
			spec_therm_cond = self.calc_species_therm_cond(spec_dyn_visc=spec_dyn_visc,
		...
		if self.num_species_full == 1:
			...
			mix_therm_cond = 0.5 * (np.sum(mole_fracs * spec_therm_cond, axis=0)
	...
	def calc_sound_speed(self, temperature,
		...
		if r_mix is None:
			assert (mass_fracs is not None), \
				"Must provide mass fractions to calculate mixture gas constant"
	...
	def calc_dens_derivs(self, density,
		...
		if wrt_spec:
			...
			if mix_mol_weight is None:
				assert (mass_fracs is not None), \
					"Must provide mass fractions to calculate mixture mol weight"
				mix_mol_weight = self.calc_mix_mol_weight(mass_fracs)

			d_dens_d_mass_frac = \
				np.zeros((self.num_species, density.shape[0]), dtype=REAL_TYPE)
			for spec_idx in range(self.num_species):
				d_dens_d_mass_frac[spec_idx, :] = (density * mix_mol_weight
	...
	def calc_stag_enth_derivs(self, wrt_press=False,
		...
		if wrt_spec:
			...
			d_stag_enth_d_mass_frac = \
				np.zeros((self.num_species, spec_enth.shape[1]), dtype=REAL_TYPE)
