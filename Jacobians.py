...
#@profile
def calcRoeDissipation_alt(solPrim, rho, h0, c, R, Cp, gas):
    
	dissMat = np.zeros((gas.numEqs, gas.numEqs, solPrim.shape[0]), dtype = constants.floatType)
...
def calc_dsolConsdsolPrim_imag(solCons,solPrim,gas):
    ...
    for i in range(solPrim.shape[0]):
        for j in range(gas.numEqs):
            ...
            gamma_matrix[:,j,i] = solCons_curr[i,:].imag/dx
...
#@profile
def calc_dFluxdsolPrim(solConsL, solPrimL, solConsR, solPrimR, sol, bounds, geoms, gas):
    ...
    dFluxdQp_l[1,1,:] = dFluxdQp_l[1,1,:] + (4.0/3.0) * gas.muRef[:-1]  / geoms.dx
    dFluxdQp_r[1,1,:] = dFluxdQp_r[1,1,:] - (4.0/3.0) * gas.muRef[:-1]  / geoms.dx
    
    dFluxdQp_l[2,2,:] = dFluxdQp_l[2,2,:] + Ck[:-1] / geoms.dx
    dFluxdQp_r[2,2,:] = dFluxdQp_r[2,2,:] - Ck[1:] / geoms.dx
...
### RHS Jacobian ###
#@profile    
def calc_dresdsolPrim(sol, gas, geom, params, bounds, dt_inv, dtau_inv):
...
def calc_dresdsolPrim_imag(sol, gas, geom, params, bounds, dt_inv, dtau_inv):
    ...
    for i in range(nsamp):
        for j in range(neq):
            ...
            #adding complex perturbations
            sol_curr.solPrim[i,j] = sol_curr.solPrim[i,j] + complex(0,dx)
...
def vec_assemble(mat):
    ...
    if numEqs > 4:
        ...
        column_curr = np.vstack((np.hstack((np.diag(mat[0,0,:],k=0),np.diag(mat[0,1,:],k=0),np.diag(mat[0,2,:],k=0),np.diag(mat[0,3,:],k=0))),np.hstack((np.diag(mat[1,0,:],k=0),np.diag(mat[1,1,:],k=0),np.diag(mat[1,2,:],k=0),np.diag(mat[1,3,:],k=0))),np.hstack((np.diag(mat[2,0,:],k=0),np.diag(mat[2,1,:],k=0),np.diag(mat[2,2,:],k=0),np.diag(mat[2,3,:],k=0))),np.hstack((np.diag(mat[3,0,:],k=0),np.diag(mat[3,1,:],k=0),np.diag(mat[3,2,:],k=0),np.diag(mat[3,3,:],k=0)))))
