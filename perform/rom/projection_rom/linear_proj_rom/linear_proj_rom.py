...
class LinearProjROM(ProjectionROM):
    ...
    def __init__(self, model_idx, rom_domain, sol_domain):
        ...
        if rom_domain.hyper_reduc:
            hyper_reduc_basis = np.load(
                rom_domain.hyper_reduc_files[self.model_idx])
            ...
            assert (hyper_reduc_basis.shape[:2]
                ...
                "Hyper reduction basis must have shape"
                + " [num_eqs, num_cells, numHRModes]")
            ...
            self.hyper_reduc_basis = (
                np.reshape(hyper_reduc_basis,
                           (-1, self.hyper_reduc_dim),
                           order="C"))

            # indices for sampling flattened hyper_reduc_basis
            self.direct_hyper_reduc_samp_idxs = np.zeros(
                rom_domain.num_samp_cells * self.num_vars, dtype=np.int32)
            for var_num in range(self.num_vars):
                ...
                self.direct_hyper_reduc_samp_idxs[idx1:idx2] = (
                    rom_domain.direct_samp_idxs
                    + var_num * sol_domain.mesh.num_cells)
    ...
    def init_from_sol(self, sol_domain):
        ...
        if self.target_cons:
            ...
            self.code = self.project_to_low_dim(
                self.trial_basis, sol, transpose=True)
            sol_domain.sol_int.sol_cons[self.var_idxs, :] = (
