...
class InviscFlux(Flux):
    ...
    def calc_d_inv_flux_d_sol_prim(self, sol):
        ...
        d_flux_d_sol_prim = np.zeros(
            (gas.num_eqs, gas.num_eqs, sol.num_cells), dtype=REAL_TYPE)
        ...
        (d_rho_d_press,
         d_rho_d_temp,
         d_rho_d_mass_frac) = gas.calc_dens_derivs(
             rho,
        ...
        (d_enth_d_press,
         d_enth_d_temp,
         d_enth_d_mass_frac) = gas.calc_stag_enth_derivs(
             wrt_press=True,
        ...
        # energy equation row
        d_flux_d_sol_prim[2, 0, :] = (
            vel * (h0 * d_rho_d_press + rho * d_enth_d_press))
        d_flux_d_sol_prim[2, 1, :] = rho * (vel_sq + h0)
        d_flux_d_sol_prim[2, 2, :] = (
            vel * (h0 * d_rho_d_temp + rho * d_enth_d_temp))
        ...
        d_flux_d_sol_prim[3:, 0, :] = (
            mass_fracs * (d_rho_d_press * vel)[None, :])
        d_flux_d_sol_prim[3:, 1, :] = mass_fracs * rho[None, :]
        d_flux_d_sol_prim[3:, 2, :] = (
            mass_fracs * (d_rho_d_temp * vel)[None, :])
        # TODO: vectorize
