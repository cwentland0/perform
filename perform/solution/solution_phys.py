...
class SolutionPhys:
    ...
    def __init__(self, gas, num_cells, sol_prim_in=None, sol_cons_in=None):
        ...
        self.sol_prim = np.zeros(
            (self.gas_model.num_eqs, num_cells), dtype=REAL_TYPE)
        self.sol_cons = np.zeros(
            (self.gas_model.num_eqs, num_cells), dtype=REAL_TYPE)

        # all species mass fractions, avoid calculation of last species
        self.mass_fracs_full = np.zeros(
            (self.gas_model.num_species_full, num_cells), dtype=REAL_TYPE)
        ...
        self.d_rho_d_mass_frac = np.zeros(
            (self.gas_model.num_species, num_cells), dtype=REAL_TYPE)
        self.d_enth_d_press = np.zeros(num_cells, dtype=REAL_TYPE)
        self.d_enth_d_temp = np.zeros(num_cells, dtype=REAL_TYPE)
        self.d_enth_d_mass_frac = np.zeros(
            (self.gas_model.num_species, num_cells), dtype=REAL_TYPE)
        ...
        if sol_prim_in is not None:
            ...
            raise ValueError("Must provide either sol_prim_in "
                             + "or sol_cons_in to solutionPhys")
    ...
    def update_state(self, from_cons=True):
        ...
        if from_cons:
            self.calc_state_from_cons(
                calc_r=True, calc_cp=True, calc_gamma=True)
        else:
            self.calc_state_from_prim(
                calc_r=True, calc_cp=True, calc_gamma=True)

    def calc_state_from_cons(self,
                             calc_r=False, calc_cp=False, calc_gamma=False):
        """
        ...
        mass_fracs = self.gas_model.calc_all_mass_fracs(
            mass_fracs, threshold=True)
        ...
        if calc_gamma:
            self.gamma_mix = self.gas_model.calc_mix_gamma(
        ...
        self.sol_prim[2, :] = (
            (self.sol_cons[2, :] / self.sol_cons[0, :]
             - np.square(self.sol_prim[1, :]) / 2.0
             - self.enth_ref_mix) / (self.cp_mix - self.r_mix))
        self.sol_prim[0, :] = (
            self.sol_cons[0, :] * self.r_mix * self.sol_prim[2, :])
    ...
    def calc_state_from_prim(self,
        ...
        # TODO: gas_model references
        self.sol_cons[0, :] = (self.sol_prim[0, :]
                               / (self.r_mix * self.sol_prim[2, :]))
        self.sol_cons[1, :] = self.sol_cons[0, :] * self.sol_prim[1, :]
        self.sol_cons[2, :] = (
            (self.sol_cons[0, :] * (self.enth_ref_mix
             + self.cp_mix * self.sol_prim[2, :]
             + np.power(self.sol_prim[1, :], 2.0) / 2.0))
            - self.sol_prim[0, :])
    ...
    def calc_state_from_rho_h0(self):
        ...
        while ((np.any(np.absolute(d_press / self.sol_prim[0, :]) > 0.01)
            ...
            h0_curr = self.gas_model.calc_stag_enth(self.sol_prim[1, :],
                                                    self.mass_fracs_full,
                                                    spec_enth=hi_curr)
            ...
            d_dens_d_press, d_dens_d_temp = (
                self.gas_model.calc_dens_derivs(
                    dens_curr,
                    wrt_press=True, pressure=self.sol_prim[0, :],
                    wrt_temp=True, temperature=self.sol_prim[2, :]))

            d_stag_enth_d_press, d_stag_enth_d_temp = (
                self.gas_model.calc_stag_enth_derivs(
                    wrt_press=True,
                    wrt_temp=True, mass_fracs=self.mass_fracs_full))

            # Compute change in temperature and pressure
            d_factor = 1.0 / (d_dens_d_press * d_stag_enth_d_temp
                              - d_dens_d_temp * d_stag_enth_d_press)
            d_press = d_factor * (d_dens * d_stag_enth_d_temp
                                  - d_stag_enth * d_dens_d_temp)
            d_temp = d_factor * (-d_dens * d_stag_enth_d_press
                                 + d_stag_enth * d_dens_d_press)

            # Threshold change in temperature and pressure
            d_press = (
                np.copysign(ones_vec, d_press)
                * np.minimum(np.absolute(d_press), self.sol_prim[0, :] * 0.1))
            d_temp = (
                np.copysign(ones_vec, d_temp)
                * np.minimum(np.absolute(d_temp), self.sol_prim[2, :] * 0.1))
