...
class FieldPlot(Visualization):
	...
	def __init__(self, image_output_dir, vis_id, vis_interval, num_steps,
				sim_type, vis_vars, vis_x_bounds, vis_y_bounds, species_names):
	...
	def plot(self, sol_prim, sol_cons, source, rhs,
		...
		for col_idx, col in enumerate(ax_list):
			...
			for row_idx, ax_var in enumerate(col_list):
				...
				lin_idx = np.ravel_multi_index(([col_idx], [row_idx]),
												(self.num_rows, self.num_cols))[0]
				if (lin_idx + 1) > self.num_subplots:
					ax_var.axis("off")
					break

				y_data = self.get_y_data(sol_prim, sol_cons,
										source, rhs, self.vis_vars[lin_idx],
										gas)
