...
class SolutionPhys:
	...
	def __init__(self, gas, num_cells, sol_prim_in=None, sol_cons_in=None):
		...
		self.sol_prim = \
			np.zeros((self.gas_model.num_eqs, num_cells), dtype=REAL_TYPE)
		self.sol_cons = \
			np.zeros((self.gas_model.num_eqs, num_cells), dtype=REAL_TYPE)

		# all species mass fractions, to avoid repeated calculation of last species
		self.mass_fracs_full = \
			np.zeros((self.gas_model.num_species_full, num_cells), dtype=REAL_TYPE)
	...
	def calc_state_from_cons(self, calc_r=False, calc_cp=False, calc_gamma=False):
		...
		self.sol_prim[2, :] = (self.sol_cons[2, :] / self.sol_cons[0, :]
	...
	def calc_state_from_rho_h0(self):
		...
		while ((np.any(np.absolute(d_press / self.sol_prim[0, :]) > 0.01)
			...
			d_stag_enth_d_press, d_stag_enth_d_temp = \
				self.gas_model.calc_stag_enth_derivs(wrt_press=True,
			...
			d_press = (np.copysign(ones_vec, d_press)
						* np.minimum(np.absolute(d_press), self.sol_prim[0, :] * 0.1))
			d_temp = (np.copysign(ones_vec, d_temp)
