...
class SolutionDomain:
	...
	def __init__(self, solver):
		...
		# gas model
		gas_file = str(param_dict["gas_file"]) 		# gas properties file (string)
	...
	def advance_iter(self, solver):
		...
		if (not solver.run_steady): print("Iteration "+str(solver.iter))
		...
		for self.time_integrator.subiter in range(self.time_integrator.subiter_max):
			...
			if (self.time_integrator.time_type == "implicit"):
				...
				if (self.sol_int.res_norm_l2 < self.time_integrator.res_tol): break
	...
	def advance_subiter(self, solver):
		...
		if (self.time_integrator.time_type == "implicit"):
			...
			sol_int.res = np.reshape(res, (self.gas_model.num_eqs, solver.mesh.num_cells), order='C')
	...
	def calc_boundary_cells(self, solver):
		...
		self.sol_inlet.calc_boundary_state(solver, sol_prim=self.sol_int.sol_prim, sol_cons=self.sol_int.sol_cons)
		self.sol_outlet.calc_boundary_state(solver, sol_prim=self.sol_int.sol_prim, sol_cons=self.sol_int.sol_cons)
	...
	def write_final_outputs(self, solver):
		...
		if solver.solve_failed: solver.sim_type += "_FAILED"
	...
	def write_probes(self, solver):
		...
		for visVar in self.probe_vars:
			probe_file_base_name += "_"+visVar		
