...
from perform.time_integrator.explicit_integrator import ExplicitIntegrator, RKExplicit
...
class ClassicRK4InitTestCase(unittest.TestCase):
    ...
    def test_classic_rk4_init(self):
        ...
        time_int = get_time_integrator("classic_rk4", self.param_dict)
...
class ClassicRK4MethodsTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.time_int = get_time_integrator("classic_rk4", self.param_dict)
...
class SSPRK3InitTestCase(unittest.TestCase):
    ...
    def test_ssp_rk3_init(self):
        ...
        time_int = get_time_integrator("ssp_rk3", self.param_dict)
...
class SSPRK3MethodsTestCase(unittest.TestCase):
    def setUp(self):
        ...
        # set up time integrator
        self.time_int = get_time_integrator("ssp_rk3", self.param_dict)
...
class JamesonLowStoreInitTestCase(unittest.TestCase):
    def setUp(self):
        # set up param_dict
        self.param_dict = {}
        self.param_dict["dt"] = 1e-7
        self.param_dict["time_scheme"] = "jameson_low_store"
        self.param_dict["time_order"] = 4

    def test_jameson_low_store_init(self):

        time_int = get_time_integrator("jameson_low_store", self.param_dict)
...
class JamesonLowStoreMethodsTestCase(unittest.TestCase):
    def setUp(self):
        # set up param_dict
        self.param_dict = {}
        self.param_dict["dt"] = 1e-7
        ...
        # set up time integrator
        self.time_int = get_time_integrator("jameson_low_store", self.param_dict)
    ...
    def test_solve_sol_change_subiter(self):
        ...
        self.time_int.rk_rhs[0] = self.rhs.copy()
        dsol = self.time_int.solve_sol_change_subiter(self.rhs)
        self.assertTrue(np.allclose(dsol, np.array([[0, 0.75], [0.25, 1.0], [0.5, 1.25]])))

    def test_solve_sol_change_iter(self):

        self.time_int.rk_rhs = self.rk_rhs
        dsol = self.time_int.solve_sol_change_iter(self.rhs)
        self.assertTrue(np.allclose(dsol, np.array([[3.0, 6.0], [4.0, 7.0], [4.0, 8.0]])))
    ...
    def test_solve_sol_change(self):
        ...
        for self.time_int.subiter in range(self.time_int.subiter_max):
            ...
            if self.time_int.subiter == 0:
                self.assertTrue(np.allclose(dsol, np.array([[0, 7.5e-8], [2.5e-8, 1e-7], [5e-8, 1.25e-7]])))
            elif self.time_int.subiter == 1:
                self.assertTrue(
                    np.allclose(
                        dsol,
                        np.array([[(10 / 3) * 1e-8, (4 / 3) * 1e-7], [(20 / 3) * 1e-8, (5 / 3) * 1e-7], [1e-07, 2e-7]]),
                    )
                )
            elif self.time_int.subiter == 2:
                self.assertTrue(np.allclose(dsol, np.array([[1.0e-7, 2.5e-7], [1.5e-7, 3.0e-7], [1.5e-7, 3.5e-7]])))
            elif self.time_int.subiter == 3:
                self.assertTrue(np.allclose(dsol, np.array([[3.0e-7, 6.0e-7], [4.0e-7, 7.0e-7], [4.0e-7, 8.0e-7]])))
