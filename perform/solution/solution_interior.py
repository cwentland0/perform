...
class SolutionInterior(SolutionPhys):
    ...
    def __init__(self, gas, sol_prim_in, solver, num_cells, time_int):
        ...
        if solver.prim_out:
            self.prim_snap = np.zeros(
                (gas.num_eqs, num_cells, solver.num_snaps + 1),
                dtype=REAL_TYPE)
            self.prim_snap[:, :, 0] = self.sol_prim.copy()
        if solver.cons_out:
            self.cons_snap = np.zeros(
                (gas.num_eqs, num_cells, solver.num_snaps + 1),
                dtype=REAL_TYPE)
        ...
        if solver.source_out:
            self.source_snap = np.zeros(
                (gas.num_species, num_cells, solver.num_snaps),
                dtype=REAL_TYPE)
        if solver.rhs_out:
            self.rhs_snap = np.zeros(
                (gas.num_eqs, num_cells, solver.num_snaps), dtype=REAL_TYPE)
        ...
        if (time_int.time_type == "implicit") or (solver.run_steady):
            ...
            if ((len(solver.res_norm_prim) == 1)
                    and (solver.res_norm_prim[0] is None)):
            ...
            for var_idx in range(gas.num_eqs):
                if (solver.res_norm_prim[var_idx] is None):
                    ...
                    solver.res_norm_prim[var_idx] = (
                        RES_NORM_PRIM_DEFAULT[min(var_idx, 3)])

            # residual norm storage
            if time_int.time_type == "implicit":

                self.res = np.zeros(
                    (gas.num_eqs, num_cells), dtype=REAL_TYPE)
                self.res_norm_l2 = 0.0
                self.res_norm_l1 = 0.0
                self.res_norm_history = np.zeros(
                    (solver.num_steps, 2), dtype=REAL_TYPE)
                ...
                row_idxs_center = np.zeros(
                    num_elements, dtype=np.int32)
                col_idxs_center = np.zeros(
                    num_elements, dtype=np.int32)
                row_idxs_upper = np.zeros(
                    num_elements - gas.num_eqs**2, dtype=np.int32)
                col_idxs_upper = np.zeros(
                    num_elements - gas.num_eqs**2, dtype=np.int32)
                row_idxs_lower = np.zeros(
                    num_elements - gas.num_eqs**2, dtype=np.int32)
                col_idxs_lower = np.zeros(
                    num_elements - gas.num_eqs**2, dtype=np.int32)
                ...
                for i in range(gas.num_eqs):
                    for j in range(gas.num_eqs):
                        for k in range(num_cells):
                            ...
                            if k < (num_cells - 1):
                                row_idxs_upper[lin_idx_B] = (
                                    i * num_cells + k)
                                col_idxs_upper[lin_idx_B] = (
                                    j * num_cells + k + 1)
                                lin_idx_B += 1

                            if k > 0:
                                row_idxs_lower[lin_idx_C] = (
                                    i * num_cells + k)
                                col_idxs_lower[lin_idx_C] = (
                                    j * num_cells + k - 1)
                                lin_idx_C += 1

                self.jacob_row_idxs = np.concatenate(
                    (row_idxs_center, row_idxs_lower, row_idxs_upper))
                self.jacob_col_idxs = np.concatenate(
                    (col_idxs_center, col_idxs_lower, col_idxs_upper))

            # "steady" convergence measures
            if solver.run_steady:
                self.d_sol_norm_l2 = 0.0
                self.d_sol_norm_l1 = 0.0
                self.d_sol_norm_history = np.zeros((
                    solver.num_steps, 2), dtype=REAL_TYPE)

    def calc_d_sol_prim_d_sol_cons(self):
        """
        Compute the Jacobian of the conservative state
        w/r/t/ the primitive state
        ...
        gamma_matrix_inv[0, 0, :] = (
            (rho * d_enth_d_temp + d_rho_d_temp * (h0 - vel_sq)
             + np.sum(mass_fracs * (d_rho_d_mass_frac * d_enth_d_temp[None, :]
                      - d_rho_d_temp[None, :] * d_enth_d_mass_frac), axis=0))
            / d)
        ...
        gamma_matrix_inv[2, 0, :] = (
            (-d_rho_d_press * (h0 - vel_sq)
             ...
             + np.sum(mass_fracs
             * ((rho * d_rho_d_press)[None, :]
                 * d_enth_d_mass_frac
                 + d_rho_d_mass_frac
                 * (rho * d_enth_d_press - 1.0)[None, :]), axis=0)
             / rho) / d)
        gamma_matrix_inv[2, 1, :] = -vel * d_rho_d_press / d
        gamma_matrix_inv[2, 2, :] = d_rho_d_press / d
        gamma_matrix_inv[2, 3:, :] = (-((rho * d_rho_d_press)[None, :]
                                      * d_enth_d_mass_frac
                                      + d_rho_d_mass_frac
                                      * (rho * d_enth_d_press - 1.0)[None, :])
                                      / (rho * d)[None, :])
    ...
    def res_jacob_assemble(self, center_block, lower_block, upper_block):
        ...
        res_jacob = (
            csr_matrix(
                (data, (self.jacob_row_idxs, self.jacob_col_idxs)),
                shape=(self.jacob_dim, self.jacob_dim),
                dtype=REAL_TYPE))
    ...
    def calc_adaptive_dtau(self, mesh):
        """
        Adapt dtau for each cell based on
        user input constraints and local wave speed
        """
        ...
        if self.visc_flux_name != "invisc":
            ...
            self.dyn_visc_mix = (
                gas_model.calc_mix_dynamic_visc(
    ...
    def write_restart_file(self, solver):
        ...
        restartIterFile = os.path.join(
            solver.restart_output_dir, "restart_iter.dat")
    ...
    def write_steady_data(self, solver):
        ...
        steady_file = os.path.join(
            unsteady_output_dir, "steady_convergence.dat")
        if (solver.iter == 1):
        ...
        out_string = (("%8i %18.14f %18.14f\n")
                      % (solver.time_iter - 1,
                      self.d_sol_norm_l2, self.d_sol_norm_l1))
        f.write(out_string)
        f.close()

        # write field data
        sol_prim_file = os.path.join(
            unsteady_output_dir, "sol_prim_steady.npy")
        np.save(sol_prim_file, self.sol_prim)

        sol_cons_file = os.path.join(
            unsteady_output_dir, "sol_cons_steady.npy")
