...
class VisualizationGroup:
	...
	def __init__(self, sol_domain, solver):
		...
		for vis_idx in range(1, self.num_vis_plots + 1):
			...
			vis_x_bounds = catch_list(param_dict, "vis_x_bounds_" + str(vis_idx),
										[[None, None]], len_highest=len(vis_vars))
			vis_y_bounds = catch_list(param_dict, "vis_y_bounds_" + str(vis_idx),
										[[None, None]], len_highest=len(vis_vars))

			if vis_type == "field":
				self.vis_list[vis_idx - 1] = FieldPlot(solver.image_output_dir, vis_idx,
				...
				self.vis_list[vis_idx - 1] = ProbePlot(solver.image_output_dir, vis_idx,
		...
		for vis in self.vis_list:
			vis.fig, vis.ax = plt.subplots(nrows=vis.num_rows, ncols=vis.num_cols,
											num=vis.vis_id, figsize=(FIG_WIDTH_DEFAULT, FIG_HEIGHT_DEFAULT))
