...
class GalerkinProjectionLinearMethodsTestCase(unittest.TestCase):
    def setUp(self):
    ...
    def test_calc_projection(self):

        projector = self.rom_method.calc_projector(self.sol_domain, self.rom_domain.model_list[0])
        self.assertTrue(np.allclose(projector, np.eye(8)))

        low_dim_vec = self.rom_method.project_to_low_dim(projector, SOL_PRIM_IN_REACT)
        self.assertTrue(np.allclose(low_dim_vec, SOL_PRIM_IN_REACT.ravel(order="C")))

        low_dim_vec = self.rom_method.project_to_low_dim(projector, SOL_PRIM_IN_REACT, transpose=True)
        self.assertTrue(np.allclose(low_dim_vec, SOL_PRIM_IN_REACT.ravel(order="C")))
    ...
    def test_calc_d_code(self):
        ...
        self.assertTrue(np.allclose(lhs, res_jacob))
        self.assertTrue(np.allclose(rhs, res.ravel(order="C")))


class GalerkinProjectionAutoencoderMethodsTestCase(unittest.TestCase):
    def setUp(self):
        ...
        # generate input text files
        solution_domain_setup()
        rom_domain_setup(method="galerkin", space_mapping="autoencoder", var_mapping="conservative")
