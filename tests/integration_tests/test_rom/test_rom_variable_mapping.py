...
class RomPrimVarMappingMethodsTestCase(unittest.TestCase):
    ...
    def test_prim_var_map_get_variables(self):
        ...
        self.assertTrue(np.array_equal(sol_prim, SOL_PRIM_IN_REACT))

        # check get_variable_hist_from_state_hist
        sol_prim_hist = self.var_mapping.get_variable_hist_from_state_hist(self.sol_domain)
        self.assertTrue(all([np.array_equal(sol_prim_arr, SOL_PRIM_IN_REACT) for sol_prim_arr in sol_prim_hist]))
    ...
    def test_prim_var_map_update_state(self):
        ...
        self.assertTrue(np.array_equal(self.sol_domain.sol_int.sol_prim, sol_prim_new))
        ...
        self.assertTrue(np.array_equal(self.sol_domain.sol_int.sol_hist_prim[0], sol_prim_new))
        self.assertTrue(np.array_equal(self.sol_domain.sol_int.sol_hist_prim[1], SOL_PRIM_IN_REACT))
        self.assertTrue(np.array_equal(self.sol_domain.sol_int.sol_hist_prim[2], SOL_PRIM_IN_REACT))
        self.assertTrue(np.array_equal(self.sol_domain.sol_int.sol_hist_cons[0], self.sol_domain.sol_int.sol_cons))
        self.assertTrue(np.array_equal(self.sol_domain.sol_int.sol_hist_cons[1], sol_cons_old))
        self.assertTrue(np.array_equal(self.sol_domain.sol_int.sol_hist_cons[2], sol_cons_old))
