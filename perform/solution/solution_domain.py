...
from perform.input_funcs import (
    get_initial_conditions, catch_list,
...
from perform.reaction.finite_rate_global_reaction import (
    FiniteRateGlobalReaction)
...
class SolutionDomain:
    ...
    def __init__(self, solver):
        ...
        if reaction_model_name == "none":
            assert (solver.source_off), (
                "Must provide a valid reaction_model_name"
                + " if source_off = False")
        ...
        # flux scheme
        self.invisc_flux_name = catch_input(
            param_dict, "invisc_flux_scheme", "roe")
        self.visc_flux_name = catch_input(
            param_dict, "visc_flux_scheme", "invisc")
        ...
        if self.invisc_flux_name == "roe":
            ...
            ones_prof = np.ones(
                (self.gas_model.num_eqs, self.sol_int.num_cells + 1),
                dtype=REAL_TYPE)
            self.sol_ave = SolutionPhys(
                gas, self.sol_int.num_cells + 1, sol_prim_in=ones_prof)
        else:
        ...
        if self.space_order > 1:
            self.grad_limiter_name = catch_input(
                param_dict, "grad_limiter", "")
        ...
        ones_prof = np.ones(
            (self.gas_model.num_eqs, self.sol_int.num_cells + 1),
            dtype=REAL_TYPE)
        self.sol_left = SolutionPhys(
            gas, self.sol_int.num_cells + 1, sol_prim_in=ones_prof)
        self.sol_right = SolutionPhys(
            gas, self.sol_int.num_cells + 1, sol_prim_in=ones_prof)
        ...
        self.sol_prim_full = np.zeros(
                ...
                dtype=REAL_TYPE)
        self.sol_cons_full = np.zeros(
            self.sol_prim_full.shape, dtype=REAL_TYPE)
        ...
        if ((self.probe_locs[0] is not None)
                and (self.probe_vars[0] is not None)):
            ...
            for idx, probe_loc in enumerate(self.probe_locs):
                if probe_loc > self.mesh.x_right:
                    ...
                    self.probe_idxs[idx] = np.abs(self.mesh.x_cell
                                                  - probe_loc).argmin()

            assert (
                not ((("outlet" in self.probe_secs)
                     or ("inlet" in self.probe_secs))
                     and (("source" in self.probe_vars)
                          or ("rhs" in self.probe_vars)))), (
    ...
    def advance_iter(self, solver):
        ...
        for self.time_integrator.subiter in (
            ...
            if self.time_integrator.time_type == "implicit":
                self.sol_int.calc_res_norms(
                    solver, self.time_integrator.subiter)
        ...
        if solver.run_steady:
            self.sol_int.calc_d_sol_norms(
                solver, self.time_integrator.time_type)
    ...
    def advance_subiter(self, solver):
        ...
        if self.time_integrator.time_type == "implicit":
            ...
            if self.time_integrator.dual_time:
                sol_int.sol_prim += d_sol.reshape(
                    (gas_model.num_eqs, mesh.num_cells), order='C')

            else:
                sol_int.sol_cons += d_sol.reshape(
                    (gas_model.num_eqs, mesh.num_cells), order='C')

            sol_int.update_state(
                from_cons=(not self.time_integrator.dual_time))
            ...
            sol_int.res = np.reshape(
                res, (gas_model.num_eqs, mesh.num_cells), order='C')
    ...
    def calc_rhs(self, solver):
        ...
        if (direct_samp_idxs[0] == 0):
            sol_inlet.calc_boundary_state(
                solver.sol_time, self.space_order,
        ...
        if (self.space_order > 1):
            ...
            sol_left.sol_prim[:, self.flux_left_extract] += (
                (self.mesh.dx / 2.0)
                * sol_prim_grad[:, self.grad_left_extract])
            sol_right.sol_prim[:, self.flux_right_extract] -= (
                (self.mesh.dx / 2.0)
                * sol_prim_grad[:, self.grad_right_extract])
        ...
        if not solver.source_off:
            source, wf = self.reaction_model.calc_source(
                self.sol_int, solver.dt, direct_samp_idxs)
            sol_int.source[
                gas.mass_frac_slice[:, None],
                direct_samp_idxs[None, :]] = source

            sol_int.wf[:, direct_samp_idxs] = wf

            sol_int.rhs[3:, direct_samp_idxs] += (
                sol_int.source[:, direct_samp_idxs])
    ...
    def calc_cell_gradients(self):
        ...
        sol_prim_grad = np.zeros(
            (self.gas_model.num_eqs, self.num_grad_cells),
            dtype=REAL_TYPE)
    ...
    def calc_res_jacob(self, solver):
        ...
        (sol_int.d_rho_d_press,
         sol_int.d_rho_d_temp,
         sol_int.d_rho_d_mass_frac) = (
        ...
        (sol_int.d_enth_d_press,
         sol_int.d_enth_d_temp,
         sol_int.d_enth_d_mass_frac) = (
        ...
        if self.time_integrator.dual_time:
            ...
            d_rhs_d_sol_prim += (gamma_matrix * (dtau_inv[None, None, :]
                                 + dt_inv))

            # Assemble sparse Jacobian from
            # main, upper, and lower block diagonals
            res_jacob = sol_int.res_jacob_assemble(
                d_rhs_d_sol_prim,
                d_flux_d_sol_prim_left,
                d_flux_d_sol_prim_right)
            ...
            gamma_matrix_inv = np.transpose(
                sol_int.calc_d_sol_prim_d_sol_cons(),
                axes=(2, 0, 1))
            d_rhs_d_sol_cons = np.transpose(
                np.transpose(d_rhs_d_sol_prim, axes=(2, 0, 1))
                @ gamma_matrix_inv,
                axes=(1, 2, 0))
            d_flux_d_sol_cons_left = np.transpose(
                np.transpose(d_flux_d_sol_prim_left, axes=(2, 0, 1))
                @ gamma_matrix_inv[:-1, :, :],
                axes=(1, 2, 0))
            d_flux_d_sol_cons_right = np.transpose(
                np.transpose(d_flux_d_sol_prim_right, axes=(2, 0, 1))
                @ gamma_matrix_inv[1:, :, :],
                axes=(1, 2, 0))
    ...
    def write_iter_outputs(self, solver):
        ...
        # Write restart files
        if (solver.save_restarts
                and ((solver.iter % solver.restart_interval) == 0)):
