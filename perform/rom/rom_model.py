...
class RomModel():
    ...
    def __init__(self, model_idx, rom_domain, sol_domain):
        ...
        self.var_idxs = np.array(
            rom_domain.model_var_idxs[self.model_idx], dtype=np.int32)
        ...
        if rom_domain.has_cons_norm:
            self.norm_sub_prof_cons = self.load_standardization(
                os.path.join(
                    self.model_dir,
                    rom_domain.norm_sub_cons_in[self.model_idx]),
                default="zeros")

            self.norm_fac_prof_cons = self.load_standardization(
                os.path.join(
                    self.model_dir,
                    rom_domain.norm_fac_cons_in[self.model_idx]),
                default="ones")
        ...
        if rom_domain.has_prim_norm:
            self.norm_sub_prof_prim = self.load_standardization(
                os.path.join(
                    self.model_dir,
                    rom_domain.norm_sub_prim_in[self.model_idx]),
                default="zeros")

            self.norm_fac_prof_prim = self.load_standardization(
                os.path.join(
                    self.model_dir,
                    rom_domain.norm_fac_prim_in[self.model_idx]),
                default="ones")
        ...
        if rom_domain.has_cons_cent:
            if rom_domain.cent_ic:
                self.cent_prof_cons = (
                    sol_domain.sol_int.sol_cons[self.var_idxs, :].copy())

            else:
                self.cent_prof_cons = self.load_standardization(
                    os.path.join(
                        self.model_dir,
                        rom_domain.cent_cons_in[self.model_idx]),
                    default="zeros")

        if rom_domain.has_prim_cent:
            if rom_domain.cent_ic:
                self.cent_prof_prim = (
                    sol_domain.sol_int.sol_prim[self.var_idxs, :].copy())
            else:
                self.cent_prof_prim = self.load_standardization(
                    os.path.join(
                        self.model_dir,
                        rom_domain.cent_prim_in[self.model_idx]),
                    default="zeros")
    ...
    def standardize_data(self, arr,
                         normalize=True,
                         norm_fac_prof=None, norm_sub_prof=None,
        ...
        if inverse:
            if normalize:
                arr = self.normalize(
                    arr, norm_fac_prof, norm_sub_prof, denormalize=True)
            if center:
            ...
            if normalize:
                arr = self.normalize(
                    arr, norm_fac_prof, norm_sub_prof, denormalize=False)
