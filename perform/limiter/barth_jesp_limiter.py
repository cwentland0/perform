...
class BarthJespLimiter(Limiter):
    ...
    def calc_limiter(self, sol_domain, grad):
        ...
        phi_left[cond1_left] = np.minimum(
            1.0, (sol_prim_max[cond1_left] - sol_prim[cond1_left]) / (sol_prim_left[cond1_left] - sol_prim[cond1_left])
        )
        phi_right[cond1_right] = np.minimum(
            1.0,
            (sol_prim_max[cond1_right] - sol_prim[cond1_right]) / (sol_prim_right[cond1_right] - sol_prim[cond1_right]),
        )
        phi_left[cond2_left] = np.minimum(
            1.0, (sol_prim_min[cond2_left] - sol_prim[cond2_left]) / (sol_prim_left[cond2_left] - sol_prim[cond2_left])
        )
        phi_right[cond2_right] = np.minimum(
            1.0,
            (sol_prim_min[cond2_right] - sol_prim[cond2_right]) / (sol_prim_right[cond2_right] - sol_prim[cond2_right]),
        )
