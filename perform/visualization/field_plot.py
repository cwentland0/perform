...
class FieldPlot(Visualization):
    ...
    def __init__(self, image_output_dir, vis_id, vis_interval,
                 num_steps, sim_type, vis_vars,
                 vis_x_bounds, vis_y_bounds, species_names):
        ...
        assert (vis_vars[0] is not None), (
            "Must provide vis_vars for field plot " + str(self.vis_id))

        self.num_imgs = int(num_steps / vis_interval)
        if self.num_imgs > 0:
            self.img_string = (
                '%0' + str(floor(log(self.num_imgs, 10)) + 1) + 'd')
        else:
    ...
    def plot(self, sol_prim, sol_cons, source, rhs,
             gas, x_cell, line_style, first_plot):
        """
        ...
        for col_idx, col in enumerate(ax_list):
            ...
            for row_idx, ax_var in enumerate(col_list):
                ...
                y_data = self.get_y_data(
                    sol_prim, sol_cons, source,
                    rhs, self.vis_vars[lin_idx], gas)
        ...
        try:
            if (var_str == "pressure"):
                ...
                if (spec_idx == gas.num_species_full):
                    massFracs = gas.calc_all_mass_fracs(
                        sol_prim[3:, :], threshold=False)
                    y_data = massFracs[-1, :]
                else:
            ...
            print(e)
            raise ValueError("Invalid field visualization variable:"
                             + str(var_str))
