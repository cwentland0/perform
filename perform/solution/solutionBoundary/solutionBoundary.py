...
class SolutionBoundary(SolutionPhys):
	...
	def __init__(self, gas, solver, bound_type):
		...
		assert (len(self.mass_fracs) == gas.numSpeciesFull), "Must provide mass fraction state for all species at boundary"
		assert (np.sum(self.mass_fracs) == 1.0), "Boundary mass fractions must sum to 1.0"

		self.cp_mix       = gas.calcMixCp(self.mass_fracs[gas.mass_frac_slice, None])
		self.r_mix        = gas.calcMixGasConstant(self.mass_fracs[gas.mass_frac_slice, None])
		self.gamma_mix    = gas.calcMixGamma(self.r_mix, self.cp_mix)
		self.enth_ref_mix = gas.calcMixEnthRef(self.mass_fracs[gas.mass_frac_slice, None])

		# this will be updated at each iteration, just initializing now
		# TODO: number of ghost cells should not always be one
		sol_dummy      = np.zeros((gas.numEqs,1), dtype=REAL_TYPE)
		sol_dummy[0,0] = 1e6
		sol_dummy[1,0] = 1.0
		sol_dummy[2,0] = 300.0
		sol_dummy[3,0] = 1.0
