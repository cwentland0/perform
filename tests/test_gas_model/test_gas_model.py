...
class GasModelInitTestCase(unittest.TestCase):
    ...
    def test_gas_model_init(self):
        ...
        mix_mass_matrix = np.array([[1.0, 0.9671682, 1.057371],
                                    [1.0339463, 1.0, 1.0932651],
                                    [0.9457416, 0.9146912, 1.0]])
        self.assertTrue(np.allclose(gas.mix_mass_matrix, mix_mass_matrix))

        mix_inv_mass_matrix = np.array([[0.25, 0.2415229, 0.2635231],
                                        [0.2581989, 0.25, 0.27116307],
                                        [0.2357023, 0.2268713, 0.25]])
...
class GasModelMethodsTestCase(unittest.TestCase):
    ...
    def test_get_mass_frac_array(self):
        ...
        self.assertTrue(np.array_equal(mass_fracs_out, np.repeat(np.array([[0.231537], [0.755063]], dtype=REAL_TYPE), repeats=2, axis=1)))
    ...
    def test_calc_mix_mol_weight(self):
        ...
        self.assertTrue(np.allclose(mix_mol_weight, np.repeat(np.array([[28.9543985]], dtype=REAL_TYPE), repeats=2, axis=1)))

    def test_calc_all_mole_fracs(self):

        mole_fracs = self.gas.calc_all_mole_fracs(self.mass_fracs)
        self.assertTrue(np.allclose(mole_fracs, np.repeat(np.array([[0.20950046], [0.78079982], [0.00969972]], dtype=REAL_TYPE), repeats=2, axis=1)))
