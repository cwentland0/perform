...
class adaptROM():
    ...
    def gatherStaleCons(self, romDomain, solDomain, solver, model):
        ...
        self.QWindow[:, :, 1:romDomain.timeIntegrator.staleStatetimeOrder+1] = np.flip(romDomain.staleSnapshots[:, :, -romDomain.timeIntegrator.staleStatetimeOrder:], axis = 2)
    ...
    #     #TODO: currently solHistPrim is not being updated. Required if solving based on primitive variables...
        ...
        for timeIdx in range(1, timeOrder + 1):
            ...
            solCons = model.standardizeData(solCons[model.varIdxs, :], normalize=True, normFacProf=model.normFacProfCons,
    ...
    #         solInt = solDomain.solInt
            ...
            self.QWindow[:, :, 0] = solDomain.solInt.solCons[model.varIdxs, :]
