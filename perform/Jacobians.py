...
# TODO: probably a lot of repeated calculations of full mass_fracs
...
def calc_d_sol_prim_d_sol_cons(sol_int):
	...
	d_rho_d_press = sol_int.d_rho_d_press; d_rho_d_temp = sol_int.d_rho_d_temp; d_rho_d_mass_frac = sol_int.d_rho_d_mass_frac
	d_enth_d_press = sol_int.d_enth_d_press; d_enth_d_temp = sol_int.d_enth_d_temp; d_enth_d_mass_frac = sol_int.d_enth_d_mass_frac
	h0 = sol_int.h0
	...
	# density row
	gamma_matrix_inv[0,0,:]  = (rho * d_enth_d_temp + d_rho_d_temp * (h0 - vel_sq) + 
	...
	gamma_matrix_inv[0,3:,:] = (d_rho_d_temp[None,:] * d_enth_d_mass_frac - d_rho_d_mass_frac * d_enth_d_temp[None,:]) / d[None,:]
	...
	# energy row
	gamma_matrix_inv[2,0,:] = (-d_rho_d_press * (h0 - vel_sq) - (rho * d_enth_d_press - 1.0) + 
							np.sum(mass_fracs * ((rho * d_rho_d_press)[None,:] * d_enth_d_mass_frac + 
							d_rho_d_mass_frac * (rho * d_enth_d_press - 1.0)[None,:]), axis=0) / rho) / d
	gamma_matrix_inv[2,1,:] = -vel * d_rho_d_press / d
	gamma_matrix_inv[2,2,:] = d_rho_d_press / d
	gamma_matrix_inv[2,3:,:] = -((rho * d_rho_d_press)[None,:] * d_enth_d_mass_frac + d_rho_d_mass_frac * 
	                           (rho * d_enth_d_press - 1.0)[None,:]) / (rho * d)[None,:]
...
def calc_d_source_d_sol_prim(sol_int, dt):
	...
	wf_div_rho = np.sum(wf[None,:] * gas.nu_arr[spec_idxs,None], axis=0) / rho[None,:] 	# TODO: not correct for multi-reaction
	
	d_wf_d_temp  = wf_div_rho * d_rho_d_temp[None,:] - wf * gas.act_energy / temp**2 	# negative, as activation energy already set as negative
	...
	d_source_d_sol_prim[3:,0,:]  = -gas.mol_weight_nu[gas.mass_frac_slice,None] * d_wf_d_press
...
>>>>>>> initial PEP 8 conformity
	...
	d_flux_d_sol_prim[2,3:,:] = vel[None,:] * (h0[None,:] * d_rho_d_mass_frac + rho[None,:] * d_enth_d_mass_frac)
...
def calc_d_visc_flux_d_sol_prim(sol_ave):
	...
	d_flux_d_sol_prim[2,3:,:] = sol_ave.sol_cons[[0],:] * (sol_ave.mass_diff_mix[gas.mass_frac_slice,:] * sol_ave.hi[gas.mass_frac_slice,:] - 
...
def calc_d_roe_flux_d_sol_prim(sol_domain, solver):
	...
	d_flux_left_d_sol_prim_left   = calc_d_inv_flux_d_sol_prim(sol_domain.sol_left)
	d_flux_right_d_sol_prim_right = calc_d_inv_flux_d_sol_prim(sol_domain.sol_right)

	if (solver.visc_scheme > 0):
		d_visc_flux_d_sol_prim         = calc_d_visc_flux_d_sol_prim(sol_domain.sol_ave)
	...
	d_flux_d_sol_prim = (d_flux_left_d_sol_prim_left[:,:,1:] + roe_diss[:,:,1:]) + (-d_flux_right_d_sol_prim_right[:,:,:-1] + roe_diss[:,:,:-1])
    
    # Jacobian wrt left neighbor
	d_flux_d_sol_prim_left = (-d_flux_left_d_sol_prim_left[:,:,1:-1] - roe_diss[:,:,:-2])
    
    # Jacobian wrt right neighbor
	d_flux_d_sol_prim_right = (d_flux_right_d_sol_prim_right[:,:,1:-1] - roe_diss[:,:,2:]) 
...
def calc_d_res_d_sol_prim(sol_domain, solver):
	...
	d_flux_d_sol_prim, d_flux_d_sol_prim_left, d_flux_d_sol_prim_right = calc_d_roe_flux_d_sol_prim(sol_domain, solver)
	...
	if sol_domain.time_integrator.dual_time:
		...
		if (sol_domain.time_integrator.adapt_dtau):
			...
			dtauInv = 1./sol_domain.time_integrator.dtau * np.ones(sol_int.num_cells, dtype=const.REAL_TYPE)

		d_rhs_d_sol_prim += gamma_matrix * (dtauInv[None,None,:] + dt_inv)

		# assemble sparse Jacobian from main, upper, and lower block diagonals
		res_jacob = res_jacob_assemble(d_rhs_d_sol_prim, d_flux_d_sol_prim_left, d_flux_d_sol_prim_right, sol_int)
		...
		gamma_matrix_inv = np.transpose(calc_d_sol_prim_d_sol_cons(sol_int), axes=(2,0,1))
		d_rhs_d_sol_cons = np.transpose(np.matmul(np.transpose(d_rhs_d_sol_prim, axes=(2,0,1)), gamma_matrix_inv), axes=(1,2,0))
		d_flux_d_sol_cons_left  = np.transpose(np.matmul(np.transpose(d_flux_d_sol_prim_left, axes=(2,0,1)), gamma_matrix_inv[:-1,:,:]), axes=(1,2,0))
		d_flux_d_sol_cons_right = np.transpose(np.matmul(np.transpose(d_flux_d_sol_prim_right, axes=(2,0,1)), gamma_matrix_inv[1:,:,:]), axes=(1,2,0))
		
		dtMat = np.repeat(dt_inv*np.eye(gas.num_eqs)[:,:,None], sol_int.num_cells, axis=2)
		d_rhs_d_sol_cons += dtMat

		res_jacob = res_jacob_assemble(d_rhs_d_sol_cons, d_flux_d_sol_cons_left, d_flux_d_sol_cons_right, sol_int)
...
def res_jacob_assemble(center_block, lower_block, upper_block, sol_int):
	...
	res_jacob = csr_matrix((data, (sol_int.jacob_row_idxs, sol_int.jacob_col_idxs)), shape=(jacob_dim, jacob_dim), dtype=const.REAL_TYPE) 
