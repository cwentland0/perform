...
class SolutionInlet(SolutionBoundary):
    ...
    def __init__(self, gas, solver):
        ...
        if self.bound_cond == "stagnation":
            ...
            raise ValueError("Invalid inlet boundary condition selection: "
                             + str(self.bound_cond))

        super().__init__(gas, solver, "inlet")

    def calc_stagnation_bc(self, sol_time, space_order,
        ...
        self.sol_prim[0, 0] = \
            self.press * pow(temp_bound / self.temp, gamma_mix / gamma_mix_m1)
        c_bound = sqrt(gamma_mix * r_mix * temp_bound)
        self.sol_prim[1, 0] = mach_bound * c_bound

    def calc_full_state_bc(self, sol_time, space_order,
                           sol_prim=None, sol_cons=None):
        """
