...
class RoeInviscFlux(InviscFlux):
    ...
    def calc_flux(self, sol_domain):
        ...
        sol_ave.mass_fracs_full = gas_model.calc_all_mass_fracs(
            sol_ave.sol_prim[3:, :], threshold=True)
        ...
        sol_ave.gamma_mix = gas_model.calc_mix_gamma(
            sol_ave.r_mix, sol_ave.cp_mix)
        ...
        flux_left[1, :] = (
            sol_cons_left[1, :] * sol_prim_left[1, :] + sol_prim_left[0, :])
        flux_left[2, :] = (
            sol_cons_left[0, :] * sol_left.h0 * sol_prim_left[1, :])
        flux_left[3:, :] = sol_cons_left[3:, :] * sol_prim_left[[1], :]
        flux_right[0, :] = sol_cons_right[1, :]
        flux_right[1, :] = (
            sol_cons_right[1, :] * sol_prim_right[1, :]
            + sol_prim_right[0, :])
        flux_right[2, :] = (
            sol_cons_right[0, :] * sol_right.h0 * sol_prim_right[1, :])
    ...
    def calc_roe_diss(self, sol_ave):
        ...
        diss_matrix = np.zeros(
            (gas_model.num_eqs, gas_model.num_eqs, sol_ave.num_cells),
            dtype=REAL_TYPE)
        ...
        (d_enth_d_press,
         d_enth_d_temp,
         d_enth_d_mass_frac) = (
            gas_model.calc_stag_enth_derivs(
                wrt_press=True,
        ...
        phi_star = (
            d_rho_d_press * phi + d_rho_d_temp * eta * (phi - vel_abs) / rho)
    ...
    def calc_jacob_prim(self, sol_domain):
        ...
        jacob_left_face = self.calc_d_inv_flux_d_sol_prim(
            sol_domain.sol_left)
        jacob_right_face = self.calc_d_inv_flux_d_sol_prim(
            sol_domain.sol_right)
